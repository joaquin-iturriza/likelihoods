...
dataset = "1908.08215-400k-fluct20_"
model = "mup_mlp"
model_name = 'mup_mlp'
...
for dataset in ["1908.08215-400k-fluct20_"]:
    for loss in ["HETEROSC"]:
        ...
        for N, n_nodes, n_layers, flavour in zip(Ns, n_nodess, n_layerss, flavours):
            ...
            # paramss2 = [1e-4,3.16e-4,1e-3,3.16e-3,1e-2]
            # L2s2 = [1e-6,1e-5,3.16e-5]
            ...
            paramss = [3.16e-4]#,1e-3,3.16e-3,1e-2]
            L2s = [1e-2]#1e-4,3.16e-4,1e-3,3.16e-3,1e-2]
            ...
            for L2 in L2s:
                for i, pars in enumerate(paramss, 1):
                    ...
                    params = f"""model={model} training.lr={pars} training.loss={loss} training.batchsize={BS} training.iterations=400000 \
