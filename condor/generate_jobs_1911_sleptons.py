...
dataset = "1911.12606-sleptons-200k-fluct20_"
model = "mlp"
model_name = 'mlp'
...
pattern = os.path.join(subs_dir, f"{dataset}_{model_name}_*_FINAL_l2_lr_grid_small_model_RAFAL_CFG_*.sub")
...
for dataset in ["1911.12606-sleptons-200k-fluct20_"]:
    for loss in ["MSE","HETEROSC"]:#HETEROSC
        ...
        for N, n_nodes, n_layers, flavour in zip(Ns, n_nodess, n_layerss, flavours):
            ...
            # paramss = [1.e-4, 3.16e-4, 1.e-3, 3.16e-3, 1e-2, 3.16e-2]
            # L2s = [1e-5, 3.16e-5, 1e-4, 3.16e-4, 1e-3, 3.16e-3, 1e-2, 3.16e-2]
            # paramss = [ 3.16e-3, 1e-2, ] 
            # L2s = [3.16e-4, 1e-3, ]
            #fill between 3.16e-3 and 1e-2 for lr, and between 3.16e-4 and 1e-3 for L2:
            paramss = np.logspace(-2.5,-2,6)
            L2s = np.logspace(-3.5,-3,6)
            paramss = [1.e-5, 1.e-4, 1.e-3,  1e-2,]
            L2s = [3.16e-6, 1e-5 ,3.16e-5, 1e-4, 3.16e-4, 1e-3]
            ...
            for L2 in L2s:
                for i, pars in enumerate(paramss, 1):
                    ...
                    params = f"""model={model} training.lr={pars} training.loss={loss} training.batchsize={BS} training.iterations={(80000//BS)*9000} \
                    data.dataset=[{dataset}] training.scheduler=CosineAnnealingLR training.cosanneal_warmup_steps={(80000//BS)*100} training.cosanneal_eta_min=0.00001 training.get_ID=true \
                    training.save_preds_intermediate=true model.net.hidden_channels={n_nodes} model.net.hidden_layers={n_layers} model.net.batchnorm=true model.net.BN_eps=0.001 model.net.BN_momentum=0.99 model.net.activation=elu\
                    training.regularization=L2 training.regularization_lambda={L2} data.trafos.fvs_standardized=["standardization"] data.nLL_trafos=["standardization"] \
                    ...
                    job_id = f"{i}"
                    sh_path = os.path.join(job_dir, f"job_{dataset}_{model_name}_{loss}_{job_id}_FINAL_l2_lr_grid_small_model_RAFAL_CFG_{to_scientific(N,0)}_N_{to_scientific(L2)}_L2.sh")
                    if intermediate:
                        sh_path = sh_path.replace(".sh", "_intermediate.sh")
                        exp_name += "_intermediate"
                        params += ' training.save_intermediate=true'
                    params += f" exp_name={exp_name}_FINAL_{to_scientific(L2)}_{to_scientific(pars)}_l2_lr_grid_small_model_RAFAL_CFG_{to_scientific(N,0)}_N"
...
""")
                    ...
                    sub_path = os.path.join(home_dir, f"subs/{dataset}_{model_name}_{loss}_FINAL_l2_lr_grid_small_model_RAFAL_CFG_{to_scientific(N,0)}_N_{to_scientific(L2)}_L2_{i}.sub")
                    job_name = f"job_{dataset}_{model_name}_{loss}_$(ID)_FINAL_l2_lr_grid_small_model_RAFAL_CFG_{to_scientific(N,0)}_N_{to_scientific(L2)}_L2.sh"
                    if intermediate:
...
if not matching_subs:
    ...
    if submit_now == "y":
        ...
        for sub_path in matching_subs:
            ...
            try:
                subprocess.run(
                            ["condor_submit", sub_path],#, "-name", "bigbird11.cern.ch", sub_path],
                            check=True
                        )
                submitted += 1
