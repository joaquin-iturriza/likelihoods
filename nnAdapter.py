...
import os, onnx, json, math, onnxruntime, sys, yaml
import numpy as np
from typing import Union

# preprocessing functions live in the same directory
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from preprocessing_nnAdapter import preprocess_features, undo_preprocess_nLLs, undo_preprocess_nLLs_errors
...
class NNAdapter:
    ...
    def _parseMetaData ( self ):
        ...
        data["featureMeans"]= None
        data["featureErrors"]= None
        data["nllMeans"]= None
        data["nllErrors"]= None
        data["trafos"] = None
        data["nLL_trafos"] = None
        remove_channels=[]
        import json, math
        isJoaquins = False
        ...
        for em in self.mlModel.metadata_props:
            ...
            if emkey == "remove_channels":
                ...
                values = json.loads(em.value)
                data["featureMeans"] = np.array(values["features_mean"][0])
                data["featureErrors"] = np.array(values["features_std"][0])
                data["nllMeans"] = np.array(values["nLLs_mean"][0])
                data["nllErrors"] = np.array(values["nLLs_std"][0])
            elif emkey == "run_config":
                import yaml as _yaml
                cfg = _yaml.safe_load(em.value)
                data["trafos"] = cfg.get("data", {}).get("trafos", None)
                data["nLL_trafos"] = cfg.get("data", {}).get("nLL_trafos", None)
    ...
    def postprocessJoaquin( self, arr ) -> dict:
        ...
        The NN outputs arr[:4] as the 4 preprocessed nLL deltas
        (nll_1 - nll_0) in order: [exp, obs, expA, obsA].
        We undo the nLL preprocessing and add back the mu=0 baselines.

        :param arr: the neural network output (may have >4 elements;
                    only arr[:4] are the nLL delta predictions)
        :returns: { "nll_exp_0": ..., "nll_exp_1": ...,
                "nll_obs_0": ..., "nll_obs_1": ...,
                "nllA_exp_0": ..., "nllA_exp_1": ...,
                "nllA_obs_0": ..., "nllA_obs_1": ... }
        ...
        # arr[:4] are the 4 standardized nLL deltas: [exp, obs, expA, obsA]
        deltas_prepd = np.array(arr[:4], dtype=np.float64)

        deltas = undo_preprocess_nLLs(
            deltas_prepd,
            mean=self.onnxMeta["nllMeans"],
            std=self.onnxMeta["nllErrors"],
            trafos=self.onnxMeta["nLL_trafos"],
        )
    ...
    def predict ( self, yields : Union[dict,list] ) -> dict:
        ...
        # print( f"@@predict yields {yields}" )
        scaled_yields = self.preprocess ( yields )
        # print ( f"@@predict scaled_yields {scaled_yields}" )
    ...
    def preprocessJoaquin ( self, yields : Union[dict,list] ) -> np.array:
        inp_list = yields
        if type(inp_list)==dict:
            inp_list = self._inputDictToList ( yields )
        inp_arr = np.array(inp_list, dtype=np.float64).reshape(1, -1)
        scaled, _, _ = preprocess_features(
            inp_arr,
            trafos=self.onnxMeta["trafos"],
            mean=self.onnxMeta["featureMeans"],
            std=self.onnxMeta["featureErrors"],
        )
...
if __name__ == "__main__":
    ...
    onnxFile = "models_onnx/SUSY-2018-16_Sleptons.onnx"
    ...
    data = np.load("data/1911.12606-sleptons-200k-fluct20_.npy")
    yieldss = data[0:10,:-8]
    truths = data[0:10,-8:]
    ...
    truths = data[:,-8:]
    preds = []
    for yields,truth in zip(yieldss,truths):
    ...
    pred_array = np.array([[p[k] for k in keys] for p in preds])
    truth_array = np.array(truths)
    # save the results in a file for later plotting
    # retrieve dataset name from onnxFile name, e.g. "SUSY-2018-16_Sleptons" from "models_onnx/SUSY-2018-16_Sleptons.onnx"
    dataset_name = os.path.basename(onnxFile).replace(".onnx", "")
    #create directory if it does not exist
    if not os.path.exists ( f"validations/{dataset_name}" ):
        os.makedirs ( f"validations/{dataset_name}" )
    np.savez(
        f"validations/{dataset_name}/results.npz",
            preds=pred_array,
            truths=truth_array,
        )

    # sanity check: mean absolute error and relative error per output, over full dataset
    print("\n=== SANITY CHECK ===")
    print(f"{'key':15s}  {'mean(truth)':>12s}  {'mean(pred)':>12s}  {'MAE':>10s}  {'rel.err':>10s}")
    for i, k in enumerate(keys):
        t = truth_array[:, i]
        p = pred_array[:, i]
        mae = np.mean(np.abs(p - t))
        rel = np.mean(np.abs(p - t) / np.maximum(np.abs(t), 1e-8))
        print(f"  {k:15s}  {np.mean(t):12.4f}  {np.mean(p):12.4f}  {mae:10.4f}  {rel*100:9.2f}%")
