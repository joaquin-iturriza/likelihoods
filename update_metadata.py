...
#
# Only two groups are dropped:
#   - the generation pipeline's plumbing (paths, buffers, verbosity, parallelism)
#   - the reference model's own training details (a different network)
# The data-generation settings (point counts, seeds, the sampled yield box,
# signal-leakage and SR/CR/VR smearing, fit_bkg, merged, ...) are KEPT: the
# training data was generated by that pipeline (R. Maselek) and filtered by us,
# so they describe our data. OLLL v0.1 lists them as metadata to retain. The
# single source for which keys those are is olll_metadata.GENERATION_KEYS.
...
REFERENCE_KEYS_TO_DROP = {
    # generation-pipeline plumbing: says nothing about the data
    "starting_points", "input_folder", "output_folder", "buffer_size",
    "keep_files", "processes", "spey_verbose_lvl",
    # the reference model's own network and training (a different model);
    # standardization_mean/std are its linear input scaling, superseded by ours
    "standardization_mean", "standardization_std", "model_version",
